...
Definition is_cont :=
  forall phi q', exists L, forall psi, phi and psi coincide_on L ->
  	 forall Fphi, F phi = some Fphi -> forall Fpsi, F psi = some Fpsi ->
  	 	Fphi q' = Fpsi q'.

Definition is_mod mf :=
  forall phi q', forall (psi : B), phi and psi coincide_on (mf phi q') ->
  	forall Fphi : B', F phi = some Fphi -> (forall Fpsi, F psi = some Fpsi -> Fphi q' = Fpsi q').
...
Lemma cont_to_sing (F: B -> option B'):
...
Lemma exists_modulus (F: B -> option B'):
	F is_continuous -> exists mf, mf is_modulus_of F.
Proof.
move => cont.
set R:= fun phiq L => forall psi, phiq.1 and psi coincide_on L -> forall Fphi, F phiq.1 = some Fphi -> 
...
(*
Lemma continuous_composition (F: B -> option B') (G: B' -> option B''):
