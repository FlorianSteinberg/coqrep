...
Proof.
move => [[[]]]; rewrite /F2MF/NXN_lst/N_XN_lst/=/lnm_size/=; last by move => [-> <-].
...
Definition lnm_list X (phi: names (rep_space_list X)):=
	map (fun n => (fun q => (phi (inr (inr (n, q)))).2.2)) (iota 0 (lnm_size phi)).

Lemma lnm_list_size X phi:
	@lnm_size X phi = size (lnm_list phi).
Proof. by rewrite /lnm_list size_map size_iota. Qed.

Lemma nth_prec (X: rep_space):
	(fun aK => nth (aK.1: X) aK.2) \is_prec_function.
Proof.
exists (fun psiphi p => match lnm_size (rprj psiphi) with
	| 0 => lprj psiphi p.2: answers X
	| S n => nth (lprj psiphi) (lnm_list (rprj psiphi)) p.1 p.2
end).
...
case E: (n <= size K); last first.
	...
	rewrite !nth_default => //=; last rewrite /lnm_list size_map size_iota (lnm_size_crct phinK) => //=.
rewrite /lnm_list.
have ineq: n < S (size K) by rewrite ltnS.
rewrite (nth_map 0); last rewrite size_iota (lnm_size_crct phinK) => //=.
rewrite nth_iota; last rewrite (lnm_size_crct phinK) => //=.
rewrite /rep_list in phinK.
move: phinK => [[onan [phinonan onanK]] _].
rewrite /F2MF /NXN_lst/N_XN_lst in onanK.
...
Proof.
have [/= nthM Mprop]:= nth_prec X.
exists (fun (phi: names (rep_space_prod X (rep_space_list X))) q => match q with
	| inl str => (some star, (0, some_answer X))
	| inr q' => match q' with
		| inl str => (some star, (S (lnm_size (rprj phi)), some_answer X))
		| inr p => (some star, (some_answer rep_space_nat, match p.1 with
			| 0 => lprj phi p.2
			| S n => nthM phi (n, p.2)
		end))
...
exists (fun phi q => fM' (lnm_size (rprj phi): nat) phi q).
...
rewrite /F2MF/NXN_lst/N_XN_lst/= in yaK.
have eq: a = an 0 by case: n phinn yaK => //= n phinn [-> yak].
have psinK : (psi n (rprj phi)) \is_name_of K.
	rewrite /psi/=/delta/=/rep_list/=/F2MF/=/NXN_lst/N_XN_lst/=.
...
have np:
	(name_pair (lprj phi)
		(name_pair phia0 [eta fM' (lnm_size (psi n (rprj phi))) (name_pair (lprj phi) (psi n (rprj phi)))]))
			\is_name_of (z,(a,f (z, K))) by trivial.
apply/ rlzr_val_sing; [ apply F2MF_sing | apply frlzr_rlzr; apply hMcmpt | | | ].
		exact: np.
	by rewrite feq.
rewrite (lnm_size_crct psinK)/F2MF.
